...
**Last verified:** 2026-07-07 (tick 5 — Rung IV QUICKEN micro-wins)
...
- Rung 2 TEST 🔄 (first bite, 2026-07-07 tick 6) — `tests/test_jsonld.py` is now the first automated test in the repo: parses the `application/ld+json` block in `index.html` and asserts required LocalBusiness fields for Google local-pack candidacy (address/PostalAddress shape, geo range, hasOfferCatalog children, etc.). Stdlib-only. `make test` delegates to `make test-jsonld`. Self-tested against broken inputs to confirm it's not a rubber-stamp. Money-path per STANDARDS §3 — structured data is the SEO surface for this business. Next test-worthy money paths (deferred): sitemap.xml round-trips ElementTree parse; robots.txt has `Sitemap:` line; conversion IIFE + adapter smoke (would need jsdom/playwright — out of stdlib scope).
...
Most recent through 2026-07-07 tick 5:
