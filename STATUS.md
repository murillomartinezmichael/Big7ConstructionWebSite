...
- **Live URL:** `https://www.big7construction.com/` — **Cloudflare Workers static assets** (worker `big7`, auto-deploys from GitHub `main` via Workers Builds; wrangler.jsonc assets dir `./`). Verified 2026-07-17: two-path version live, `/big7.js` 200, `/home-repair.html` 301. **Apex `big7construction.com` returns 404** — not yet bound to the worker (repo canonicals + m3mm.net pattern say apex should be canonical with www redirecting; dashboard gate in PENDING_MANUAL). Note: CF html_handling serves lane pages extensionless (`/commercial-industrial.html` 307s to `/commercial-industrial`); internal links still use `.html` — one extra hop per lane click until the extensionless migration (TODO PARKED).
