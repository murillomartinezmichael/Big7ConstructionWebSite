...
- **Live URL:** `https://big7construction.com/` — **Cloudflare Workers static assets** (worker `big7`, auto-deploys from GitHub `main` via Workers Builds; wrangler.jsonc assets dir `./`). Apex bound 2026-07-17 (was blocked on a stale `CNAME → *.up.railway.app` DNS record on the zone; deleted, then apex custom domain added clean). Verified live post-bind: `/` 200 w/ real content, `/commercial-industrial.html` 307 (extensionless), `/big7.js` 200, `/home-repair.html` 301. **`www.big7construction.com` intentionally removed** (Mike's call, same session) rather than bound + 301'd — apex is the sole host, matching the repo's existing apex-form canonicals. **[Correction 2026-08-07: www DOES resolve and serves a byte-identical 200 duplicate of apex from the Cloudflare edge (cert SAN incl. www issued 2026-07-17); no redirect in place. Disposition decision → `PENDING_MANUAL.md`.]** Note: CF html_handling serves lane pages extensionless (`/commercial-industrial.html` 307s to `/commercial-industrial`); internal links still use `.html` — one extra hop per lane click until the extensionless migration (TODO PARKED).
- **Legacy/fallback:** the nginx/Docker/Railway stack remains in-repo and CI-tested (container smoke) but is no longer the host of record. Security headers on CF come from the `_headers` file (added 2026-07-17); `_redirects` carries the home-repair 301 there.
...
- Rung 1 HARDEN ✅ (reopened + re-closed 2026-07-07 tick 15) — non-root nginx user, security headers (`Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`), no dynamic input surface (static site). **Tick 15**: writing `tests/test_nginx_headers.py` surfaced a real gap — `/404.html` location declared `add_header Cache-Control` which per nginx inheritance rules dropped all 5 server-level security add_headers from 404 responses; MITM-downgrade + iframe-spoof surface on every mistyped URL. Fix: repeat the 5 add_header directives inside `location = /404.html` matching the pattern already used for `/index.html` / `/robots.txt` / `/sitemap.xml`, plus an inline WHY comment so future edits don't strip it as "duplication." Rung now claimed by test, not by inspection.
- Rung 2 TEST 🔄 (eight bites landed, 2026-07-07 ticks 6+7+10+11+12+13+14+15) — `tests/test_jsonld.py` (ticks 6+11) parses the `application/ld+json` block in `index.html` and asserts LocalBusiness shape for Google local-pack candidacy; tick 11 added `image` + `logo` to `REQUIRED_TOP` and a `--selftest` mode (10 broken-mutation cases, all caught). `tests/test_seo_files.py` (tick 7) parses `sitemap.xml` + `robots.txt` and cross-checks canonical origin agrees across `sitemap.xml <loc>`, `robots.txt Sitemap:`, and `index.html <link rel="canonical">` — guards silent-drift. `tests/test_conversion.py` (tick 10) parses the conversion IIFE's `INTENT_TO_TYPE` mapping, all `data-intent="service:*"`/`portfolio:*"` CTAs, and all `<input name="projectType" value="...">` radios — asserts a four-way contract (every mapping value is a real radio, every CTA has a mapping, no dead mapping entries, no orphan radios) plus substring presence of `track('cta_click'` / `track('intake_submit'` / `dataLayer` / `window.gtag` / `window.plausible` to lock the attribution loop end-to-end. `tests/test_og_twitter.py` (tick 12) parses `<meta>` tags on BOTH `index.html` and `404.html`, asserts the required OG + Twitter tag set is present, locks `og:image` + `twitter:image` to the branded `og-card.png` (rejects reversion to the 206px `jobsite-01.jpg` placeholder that tick 4 moved OFF of), asserts `og:image:width == 1200` / `height == 630` / `type == image/png`, asserts `twitter:card == summary_large_image`. `tests/test_assets.py` (tick 13) walks every image URL referenced by JSON-LD `image`/`logo` + `og:image`/`twitter:image` across both HTML files, resolves each via the canonical origin to a local path, and asserts file exists + non-empty + magic-byte prefix matches the extension (PNG `\x89PNG…`, JPEG `\xff\xd8\xff`) — closes the exact `git rm images/og-card.png` gap the URL-string tests all miss. `tests/test_anchors.py` (tick 14) extracts every non-empty `href="#..."` reference and every `id="..."` attribute from `index.html` and asserts (a) every href name is in the id set (case-sensitive per HTML5 §6.9.4), (b) `#main` (WCAG 2.4.1 skip-link) + `#contact` (money anchor) are both present unconditionally, (c) `#contact` is referenced by ≥ 10 CTAs — closes the "someone renamed `<section id="contact">` and every `href="#contact"` CTA silently scrolls nowhere" regression class that the CTA-mapping test (tick 10) does not touch. `tests/test_nginx_headers.py` (tick 15) parses `nginx.conf` (with a `_strip_comments` pass so inline comments containing `location = /...` can't fool the block extractor), walks the outermost `server { ... }` block + every `location <matcher> { ... }` inside it via a brace-depth scanner, and asserts: server-scope carries all 5 defense-in-depth headers (HSTS/XCTO/XFO=DENY/Referrer-Policy=strict-origin-when-cross-origin/Permissions-Policy) plus `Vary: Accept-Encoding` plus long-cache immutable default; each of the 4 protected locations (`/index.html`, `/robots.txt`, `/sitemap.xml`, `/404.html`) repeats those 5 headers inline (because any nested `add_header` resets nginx inheritance); per-location Cache-Control matches spec (`/index.html` = short-cache, crawler + error surfaces = 1h); `/404.html` has `internal;`; `location /` `try_files` fallback ends in `=404` (not `/index.html` — the historical SEO regression this codebase already suffered once); HSTS `max-age >= 15768000` + includes `includeSubDomains`. First run against production nginx.conf caught a real Rung 1 gap (see Rung 1 line above). All seven stdlib-only. `make test` chains via `make test-jsonld test-seo-files test-conversion test-og test-assets test-anchors test-nginx` and every target runs its selftest alongside the golden check. Each self-tested against ≥8 deliberately-broken variants (tick 12: 12 mutations, tick 15: 12 mutations + 1 comment-injection probe, ticks 13/14: 8 each) — all caught, baselines PASS. Next test-worthy money paths (deferred): live form-submit + click-loop behavior (would need jsdom/Playwright — out of stdlib scope), actual HTTP-response header verification (only observable against a live container, requires docker + curl — out of stdlib scope) — see PARKED below.
