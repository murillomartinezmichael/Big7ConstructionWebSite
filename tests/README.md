...
Runnable on any Python 3.11+ machine with zero setup — `make test` chains all five.
...
Each of the five tests is self-tested against ≥6 deliberately-broken inputs (missing fields, drifted origins, orphan mappings, placeholder-image regressions, missing-on-disk assets, etc.) — never rubber-stamps.
